...
export async function matchTerritoryQueues() {
  ...
  for (const akey of keys) {
    ...
    if (attackers.length && defenders.length) {
      ...
      await db.query(`INSERT INTO territory_battles (id, world_seed, region_id, attackers, defenders, status, started_at) VALUES ($1, $2, $3, $4, $5, 'active', now())`, [id, seed, region, JSON.stringify([attackerGuild]), JSON.stringify([defenderGuild])]);
...
function cryptoRandomId() { return (globalThis.crypto?.randomUUID?.() || require('crypto').randomUUID()).replace(/-/g, ''); }
